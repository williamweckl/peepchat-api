defmodule Peepchat.RoomControllerTest do
  ...
  test "lists all entries on index", %{conn: conn, user: user} do
    ...
    assert Enum.count(json_response(conn, 200)["data"]) == Repo.aggregate(Room, :count, :id)
  end
  ...
  test "lists owned entries on index (owner_id = user id)", %{conn: conn, user: user} do
    ...
    user_room_count = Room
    |> where(owner_id: ^user.id)
    |> Repo.aggregate(:count, :id)
    assert Enum.count(json_response(conn, 200)["data"]) == user_room_count
  end
